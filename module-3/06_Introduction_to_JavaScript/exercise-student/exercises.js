...
function cigarParty(cigarNum, isWeekend){
	result = false;
	if (cigarNum > 39 && cigarNum < 61){
		result = true;
	}
	if (cigarNum > 39 && isWeekend){
		result = true;
	}
	return result;
}
...
*/
	...
	function filterBigNumbers(numArr){
		smallArr = [];
		for (let int of numArr){
			if (int > 99){
				smallArr.push(int);
...
function filterMultiplesOfX(numArr, num){
	smallArr = [];
	for (let int of numArr){
		if (int % num == 0){
			smallArr.push(int);
