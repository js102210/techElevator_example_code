    ...
    <form v-on:submit.prevent="addNewReview" v-if="showForm == true">
      <div class="form-element">
        ...
        <div class="form-element">
          ...
          <select id="rating" v-model.number= "newReview.rating">
            <option value="5">5 Stars (are you sure?)</option>
          </select>
          <div></div>
...
export default {
  ...
  data() {
    return {
      ...
      filter: 0 //if filter set to 0, show all, else show only show reviews with that rating
    };
  },
  computed: {
    averageRating() {
    ...
    filteredReviews(){
      return this.reviews.filter ( review => review.rating == this.filter || this.filter == 0)
    }
  },
  methods: {
    addNewReview(){
