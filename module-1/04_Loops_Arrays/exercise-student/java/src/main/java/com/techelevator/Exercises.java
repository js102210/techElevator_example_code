...
public class Exercises {
	...
	public int[] maxEnd3(int[] nums) {
		...
		return new int[] {largerVal, largerVal, largerVal};
	}
	...
	public boolean lucky13(int[] nums) {
	    boolean hasNoOneOrThree = true;
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] == 1 || nums[i] == 3 || nums[i] == 13) {
            	hasNoOneOrThree = false;
			}
        }
		return hasNoOneOrThree;
	}
	...
	public boolean sum28(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			...
			if ((i == nums.length -1) && (sumOfTwos == 8)) {
				return true;
			}
		}
		return false;
